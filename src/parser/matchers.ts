...
const GROUPED_CODES_LINE =
  /^\b(FWC|ALG|ANG|ARG|AUS|AUT|BEL|BIH|BRA|CAN|CIV|CMR|COD|COL|CPV|CRO|CUW|CZE|DEN|ECU|EGY|ENG|ESP|FRA|GER|GHA|HAI|IRN|IRQ|JOR|JPN|KOR|KSA|MAR|MEX|NED|NOR|NZL|PAN|PAR|POR|QAT|RSA|SCO|SEN|SUI|SWE|TUN|TUR|URU|USA|UZB|CC)\b.*?((?:\d+(?:\(x\d+\))?(?:,\s*)?)+)/gim;
const GROUPED_CODES_ITEM = /(\d+)(?:\(x(\d+)\))?/gi;

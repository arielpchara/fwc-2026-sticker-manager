...
server.tool(
  ...
  "Parse another person's sticker list and return stickers they have that you don't (what you can receive from them).",
  {
    text: z
      .string()
      .describe("Free text containing the other person's sticker codes"),
  },
  async ({ text }) => {
...
server.tool(
  ...
  "Parse surplus stickers from text (format: CODE (xN)) and update your collection. Each code gets 1 + N copies total.",
  {
    text: z
      .string()
      .describe(
        "Free text containing surplus sticker codes in CODE (xN) format",
      ),
  },
  async ({ text }) => {
    ...
    if (result.totalUnique === 0) {
      return {
        content: [
          {
            type: "text",
            text: "No extra stickers (all owned in single copies).",
          },
        ],
      };
    }
    const lines = result.items.map(
      (e) => `${e.code} x${e.qty} (surplus: ${e.surplus})`,
    );
    return {
      content: [
        {

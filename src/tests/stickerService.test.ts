...
import {
  ...
  getExtras,
  addSurplus,
} from "../core/stickerService.js";
import type { Repository } from "../core/stickerService.js";
import type { StickersRecord } from "../storage/stickersRepository.js";
import type { Inventory } from "../domain/inventory.js";
import { codesOf } from "../domain/inventory.js";

function makeRepo(): Repository {
  let store: StickersRecord = { inventory: {}, hash: "", updatedAt: "" };
  return {
    async load() {
      return { ...store, inventory: { ...store.inventory } };
    },
...
describe("stickerService", () => {
  ...
  describe("setOwn", () => {
    ...
    it("parses xN count from text", async () => {
      ...
      expect(record.inventory).toEqual({ ARG1: 1, BRA1: 3 });
    });
  ...
  describe("getOwn", () => {
    it("returns empty state initially", async () => {
      const record = await getOwn(repo);
      expect(codesOf(record.inventory)).toEqual([]);
    });
  ...
  describe("addSurplus", () => {
    it("adds surplus to existing sticker", async () => {
      ...
      expect(result.saved).toBe(true);
      const record = await getOwn(repo);
      expect(record.inventory["RSA5"]).toBe(2);
    });
    ...
    it("adds sticker not in collection yet", async () => {
      ...
      expect(result.saved).toBe(true);
      const record = await getOwn(repo);
      expect(record.inventory["RSA5"]).toBe(3);
    });
    ...
    it("updates multiple stickers", async () => {
      ...
      const record = await getOwn(repo);
      expect(record.inventory["RSA5"]).toBe(2);
      expect(record.inventory["RSA12"]).toBe(2);
    });
    ...
    it("does not affect codes not in surplus text", async () => {
      ...
      await addSurplus("RSA5 (x1)", repo);
      const record = await getOwn(repo);
      expect(record.inventory["BRA1"]).toBe(1);
    });

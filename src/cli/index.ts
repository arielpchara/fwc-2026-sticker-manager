...
async function cmdOwn(): Promise<void> {
  ...
  if (values["list"]) {
    ...
    const stickers = codesOf(record.inventory);
    if (stickers.length === 0) {
      console.log("No stickers saved yet.");
    } else {
      const total = Object.values(record.inventory).reduce((a, b) => a + b, 0);
      console.log(
        `Your stickers (${total} copies of ${stickers.length} unique):`,
      );
      for (const code of stickers) {
        const qty = record.inventory[code];
  ...
  if (!result.saved) {
    ...
    console.log(
      `Saved ${result.count} unique stickers (${result.totalCopies} total copies):`,
    );
...
async function cmdSurplus(): Promise<void> {
  ...
  if (result.count === 0) {
    ...
    for (const code of result.updated) {
      const record = await getOwn();
      console.log(`  ${code} x${record.inventory[code]}`);
    }
  }
}
...
async function cmdExtras(): Promise<void> {
  ...
  if (result.totalUnique === 0) {
    ...
    console.log(
      `Extra stickers (${result.totalUnique} codes, ${result.totalSurplus} surplus):`,
    );

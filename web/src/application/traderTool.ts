...
export function sortByGroup(trades: TradeBy[]): TradeBy[] {
    console.log('sortByGroup', trades)
    const teamsInOrder = GROUPS.flatMap((group) => group.prefixes).reduce((acc, prefix) => ({
        ...acc,
        [prefix]: [],
    }), {
        FWC: []
    } as Record<string, TradeBy[]>)
    console.log('teamsInOrder', teamsInOrder)
...
export function updateTrade(trades: TradeBy[], index: number, newTrade: TradeBy): TradeBy[] {

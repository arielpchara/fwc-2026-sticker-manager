...
export default function CompareHistory({
  ...
  const matchingLabels = useMemo(() => {
    const giveLabels = new Set(entries.filter(e => e.mode === 'give').map(e => e.name))
    const receiveLabels = new Set(entries.filter(e => e.mode === 'receive').map(e => e.name))
          ...
          >
            ...
            <div className="flex items-center gap-1 shrink-0">
              {matchingLabels.includes(entry.name) && onTradeNavigate && (
                <button
                  onClick={(e) => { e.stopPropagation(); onTradeNavigate(entry.name) }}
